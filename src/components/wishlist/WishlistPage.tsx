...
type WishlistItem = {
  id: string;
  variantId: string;
  variant: { id: string; product: VariantProduct };
};
...
function WishlistPage() {
  ...
  if (items.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[70vh] text-center px-4">
        <div className="w-20 h-20 bg-gray-100 rounded-full mb-4 flex items-center justify-center shadow-sm">
          <Heart className="w-10 h-10 text-gray-400" />
        </div>
        <h2 className="text-lg font-semibold mb-2">Your wishlist is empty</h2>
        <p className="text-gray-600 text-sm mb-6 max-w-sm">
          Save your favorite products and easily find them later.
        </p>
        <button
          onClick={() => router.push("/")}
  ...
  return (
    <div className="page-wrap">
      {/* Header */}
      <div className="mb-8">
        <h1 className="page-title mb-6 md:mb-8">My Wishlist</h1>
