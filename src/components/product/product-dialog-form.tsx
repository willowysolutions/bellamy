...
export const ProductDialogForm = ({
  productId, // If editing, pass productId
}: {
  productId?: string;
}) => {
  ...
  const handleOptionClick = (type: "simple" | "configurable") => {
    if (type === "simple") {
      router.push(`/products/add/simple`);
    } else {
      router.push(`/products/configurable-product`);
    }
    setShowAddOptions(false); // Hide options after navigation
  };
  ...
  return (
    <div>
      ...
      {productId && (
        <Button onClick={handleEditProduct} className="ml-2">
          Edit Product
        </Button>
      )}

      {/* Show Simple/Configurable options */}
      {showAddOptions && (
        <div className="mt-4 border shadow rounded p-4 bg-white max-w-xs">
